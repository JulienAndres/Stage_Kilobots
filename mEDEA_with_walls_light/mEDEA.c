...
void genome_motion(){//controlleur
	...
	double moteur_g=1.0*(mydata->genome[0]-1)+1.0*(mydata->genome[2]-1)*moyenne+1.0*(mydata->genome[4]-1)*mydata->nb_voisins+(mydata->genome[6]-1)*is_feed()+(mydata->genome[8]-1)*hasMur(); //biais + moyenne_distance_des_voisin + nb_voisins
	double moteur_d=1.0*(mydata->genome[1]-1)+1.0*(mydata->genome[3]-1)*moyenne+1.0*(mydata->genome[5]-1)*mydata->nb_voisins+(mydata->genome[7]-1)*is_feed()+(mydata->genome[9]-1)*hasMur(); //biais + moyenne_distance_des_voisin + nb_voisins
...
void setup_message(){
	...
	int tmp3=100*(mydata->genome[6])+10*(mydata->genome[7])+mydata->genome[8];
	int tmp4=100*(mydata->genome[9]);
	...
	mydata->msg_transmis.data[4]=tmp4;//transmission genome

...
# 50 "evite.c"
 ...
 if(kilo_ticks > mydata->wall_detected_timeout+2*32) {
  mydata->wall_detected = 0;
    mydata->state = SEARCHING;
  }
  ...
  if(mydata->new_message){
    ...
    update_from_message();
    mydata->RXHead = (mydata->RXHead+1)%8;;
  } else {
...
# 70 "evite.c"
    ...
    if(kilo_ticks > mydata->rotation_timer){
      mydata->rotation_timer = kilo_ticks + 7*32;
      if(mydata->curr_motion == LEFT){
        set_motion(RIGHT,0);
      } else {
        set_motion(LEFT,0);
      }
      { mydata->RXHead = 0; mydata->RXTail = 0;};
...
uint8_t hasMur(){
  ...
  if(kilo_ticks>mydata->mur_update+32) return 0;
  if(mydata->mur_dist>100) return 0;
